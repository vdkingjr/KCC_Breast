...
Month1  = c(1:12)
mydata <- read.csv("BreastSurgT2.csv")
...
ui <- fluidPage(theme = shinytheme("readable"),
  ...
  sidebarLayout(
    ...
    mainPanel(
      # outputs here 
      plotOutput(outputId = "Brstplot"),
      plotOutput(outputId = "radio"),
      
      
      # Show data table
      dataTableOutput(outputId = "BrstDataTable")
    )
  )
)
...
server <- function(input, output, session) {
 ...
 (input$radio)
  ...
  output$BrstDataTable <- renderDataTable({
    if(input$show_data){
  ...
  output$radio <- renderPlot({
    ...
    if (input$radio == "SurgCase22 and SurgCase23 to Budget"){
      ...
      ggplot(mydata, aes(x= Month1, group = 1)) + 
  ...
  output$Brstplot <- renderPlot({
    ...
    #   scale_color_manual(values = c("blue"))
      ...
      ggplot(mydata, aes_string(x=Month1, y=input$varInput))  +
